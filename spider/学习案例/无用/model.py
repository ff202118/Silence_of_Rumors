...
node = soup.find_all('script')
str = node[2].get_text().split('\n')
str = str[0]
str = str.replace('var allData = ', '').rstrip(';')
...
# for item in json_data['newsstream']:
#     title = item['title']
#     detail_url = item['url']
#     news_time = item['newsTime']
#     print(title, detail_url, news_time)
#     # 使用XPath定位<a>元素
xpath_expression = "//h2[contains(@class, 'news-stream-newsStream-news-item-title')]/a"
elements = driver.find_element(By.XPATH, xpath_expression).click()
print(elements.text)
